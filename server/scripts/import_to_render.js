...
const prisma = new PrismaClient({
    datasources: {
        db: {
            url: 'postgresql://teammate_user:deoJFgogG9Ahl9amTSDlBXD0ci45naCE@dpg-d6fn4phaae7s73ekqga0-a/teammate_db_012h'
        }
    }
});
...
async function main() {
    ...
    for (const lead of data.leads) {
        try {
            ...
            console.log(`  ⚠️  ${lead.name}: ${e.message}`);
        }
    }
    console.log(`  ✅ ${leadsImported} leads imported`);
    ...
    for (const property of data.properties) {
        try {
            ...
            console.log(`  ⚠️  ${property.name}: ${e.message.substring(0, 50)}`);
        }
    }
    console.log(`  ✅ ${propsImported} properties imported`);
    ...
    for (const project of data.projects) {
        try {
            ...
            console.log(`  ⚠️  ${project.title}: ${e.message.substring(0, 50)}`);
        }
    }
    console.log(`  ✅ ${projectsImported} projects imported`);
    
    console.log('\n=== Import Complete ===');
    console.log(`Users: ${data.users.length} (upserted)`);
    console.log(`Leads: ${leadsImported}`);
    console.log(`Properties: ${propsImported}`);
    console.log(`Projects: ${projectsImported}`);
}
