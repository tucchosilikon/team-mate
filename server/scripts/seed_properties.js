...
async function seedProperties() {
    try {
        ...
        // Find or create an Owner Lead
        let ownerLead = await prisma.lead.findFirst({
            where: { name: 'Property Manager' }
        });

        if (!ownerLead) {
            console.log('Creating default Property Manager lead...');
            ownerLead = await prisma.lead.create({
                data: {
        ...
        console.log(`✅ Using Owner Lead: ${ownerLead.name} (${ownerLead.id})`);
